// HTML report renderer for convex-security. Self-contained, no deps.
const esc = (s) => String(s ?? "").replace(/[&<>"]/g, (c) => ({ "&": "&amp;", "<": "&lt;", ">": "&gt;", '"': "&quot;" }[c]));

const SEV_COLOR = { critical: "#dc2626", high: "#f97316", medium: "#eab308", low: "#3b82f6", info: "#6b7280" };

export function renderHtml(result) {
  const { convex_url, summary, findings, active_probe, functions_checked } = result;
  const total = findings.length;
  const score = Math.max(0, 100 - (summary.critical * 20 + summary.high * 10 + summary.medium * 4 + summary.low * 1));
  const fnCount = (functions_checked?.queries?.length || 0) + (functions_checked?.mutations?.length || 0);
  ...
  const rows = findings.map((f) => `
    <div style="border-left:4px solid ${SEV_COLOR[f.severity]};background:#fff;padding:14px 18px;border-radius:6px;margin-bottom:12px;box-shadow:0 1px 2px rgba(0,0,0,.06)">
      ...
      ${f.probe?.confirmed ? `<div style="font-size:12px;color:#b91c1c;margin-top:4px">✓ CONFIRMED via anonymous probe${f.details?.row_count != null ? ` — ${f.details.row_count} row(s) returned` : ""}${Array.isArray(f.details?.columns) && f.details.columns.length ? ` (fields: ${esc(f.details.columns.join(", "))})` : ""}</div>` : ""}
...
<title>convex-security report — ${esc(convex_url)}</title>
...
<body><div class="wrap">
  <h1 style="margin:8px 0">🛡️ Convex Security Report</h1>
  <div style="color:#6b7280;font-size:14px">${esc(convex_url)} · scored ${score}/100 · ${active_probe.confirmed} leak(s) confirmed live</div>
  <div class="card" style="margin-top:14px">
    ...
    <span class="stat"><div class="n">${fnCount}</div><div class="l">FUNCTIONS</div></span>
  </div>
  ...
  <div class="card" style="background:#ecfdf5;border:1px solid #a7f3d0">
    ...
    <p style="font-size:14px;color:#374151;margin:0 0 10px">You have <strong>${summary.critical} critical</strong> and <strong>${summary.high} high</strong> findings. I verify each one live and send a written report with the exact auth-gate fixes.</p>
    ...
    <a href="https://github.com/Perufitlife/convex-security" style="color:#047857;font-size:14px;margin-left:10px">See the tool</a>
  </div>` : ""}
  <div style="text-align:center;font-size:12px;color:#9ca3af;padding:8px">
    Generated by <a href="https://github.com/Perufitlife/convex-security" style="color:#047857">convex-security</a> · MIT · Runs locally, nothing leaves your machine.
  </div>
</div></body></html>`;
}
